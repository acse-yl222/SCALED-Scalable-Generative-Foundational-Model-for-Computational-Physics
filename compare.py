...
from src.dataset.flow_vae_dataset import FlowDiffusionDatasetFreeSamplingV5
...
def log_validation(
    ...
    back_data[:, :, 1:-1] = 1
    back_data[:, 0:1][background_value] = 1 / 3
    ...
    # pre_tem[:,:,1:-1,4:-4,4:-4] = pre_particle[:,:,1:-1,4:-4,4:-4]
...
def main(cfg):
    ...
    train_dataset = FlowDiffusionDatasetFreeSamplingV5(
        data_dir="/lustre/scratch/mmm1460/data/flow_past_building_v1",
        rotato_ratio=1,
        stride=4,
        skip_timestep=cfg.skip_timestep,
        time_steps_list=[i for i in range(200, 5000)])
    val_dataset  = FlowDiffusionDatasetFreeSamplingV5(
        data_dir="/lustre/scratch/mmm1460/data/flow_past_building_v1",
        rotato_ratio=0,
        stride=16,
        skip_timestep=cfg.skip_timestep,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./config/train_diffusion_unet_m_lossweight_stage2.yaml")
