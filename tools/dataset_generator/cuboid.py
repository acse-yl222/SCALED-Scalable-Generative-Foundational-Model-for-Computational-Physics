...
if LIBM == True:
    ...
    sigma = torch.where(sigma >0.8, torch.tensor(1e08, dtype=torch.float32, device=device), torch.tensor(0, dtype=torch.float32, device=device))
    plt.imshow(sigma[0,0,4,:,:].cpu())
    plt.colorbar()
    plt.savefig('Street_trainning.jpg')
    plt.close()
    
np.save(f'{save_path}/sigma.npy', sigma.cpu().numpy())
