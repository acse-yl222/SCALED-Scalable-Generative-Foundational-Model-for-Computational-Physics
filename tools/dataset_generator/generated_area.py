...
if LIBM == True:
    ...
    for i in range(nz):
        sigma[0,0,i,:,:] = torch.tensor(mesh[0,0,i,:,:])
    sigma = sigma.transpose_(4, 3)
    sigma = torch.flip(sigma, [3])
    sigma = torch.where(sigma >0.9, torch.tensor(1e08, dtype=torch.float32, device=device), torch.tensor(0, dtype=torch.float32, device=device))
    plt.imshow(sigma[0,0,4,:,:].cpu())
    plt.colorbar()
    plt.savefig('generated_area.jpg')
    plt.close()
    
np.save('/lustre/scratch/mmm1460/data/flow_past_generated_area_v1/sigma.npy', sigma.cpu().numpy())
...
# output
        ...
        if np.max(np.abs(w.cpu().detach().numpy())) > 80000.0:
            ...
            break
        if save_fig == True:
            np.save(f"{save_path}/w"+str(itime), arr=values_w.cpu().detach().numpy()[0,0,:,:].astype(np.float16))
            np.save(f"{save_path}/v"+str(itime), arr=values_v.cpu().detach().numpy()[0,0,:,:].astype(np.float16))
            np.save(f"{save_path}/u"+str(itime), arr=values_u.cpu().detach().numpy()[0,0,:,:].astype(np.float16))
            #np.save("/content/drive/My Drive/AI4PDE/Urban_data_L/p"+str(itime), arr=values_p.cpu().detach().numpy()[0,0,:,:])
    end = time.time()
