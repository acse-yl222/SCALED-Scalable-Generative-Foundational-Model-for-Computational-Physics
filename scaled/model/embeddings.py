...
def get_1d_sincos_pos_embed(
    ...
    grid_l = np.arange(grid_size, dtype=np.float32) / (grid_size / base_size) / interpolation_scale
    grid = np.meshgrid(grid_l)  # here w goes first
...
def get_3d_sincos_pos_embed(
    ...
    return: pos_embed: [depth*height*width, embed_dim] or [extra_tokens + depth*height*width, embed_dim] (w/ or w/o cls_token)
    """
    if isinstance(grid_size, int):
        grid_size = (grid_size, grid_size, grid_size)

    grid_d = np.arange(grid_size[0], dtype=np.float32) / (grid_size[0] / base_size) / interpolation_scale
    ...
    grid = np.meshgrid(grid_w, grid_h, grid_d, indexing='ij')  # create 3D grid
    grid = np.stack(grid, axis=0)  # stack to shape [3, width, height, depth]
...
class PatchEmbed1D(nn.Module):
    """1D Image to Patch Embedding with support for SD3 cropping."""
    ...
    def __init__(
        ...
        self.length = length // patch_size
        self.base_size = length // patch_size
    ...
    def forward(self, latent):
        ...
        length = latent.shape[-1]//self.patch_size

        latent = self.proj(latent)
        # if self.flatten:
        latent = latent.transpose(1, 2)  # BCHW -> BNC
        if self.layer_norm:
...
class PatchEmbed3D(nn.Module):
    """3D Image to Patch Embedding with support for SD3_3D cropping."""
    ...
    def __init__(
        ...
        num_patches = (depth//patch_size)*(height // patch_size) * (width // patch_size)
        self.flatten = flatten
        self.layer_norm = layer_norm
        self.pos_embed_max_size = pos_embed_max_size

        self.proj = nn.Conv3d(
            in_channels, embed_dim, kernel_size=(patch_size, patch_size,patch_size), stride=patch_size, bias=bias
        )
        if layer_norm:
            self.norm = nn.LayerNorm(embed_dim, elementwise_affine=False, eps=1e-6)
        else:
        ...
        self.patch_size = patch_size
        self.depth,self.height, self.width = depth//patch_size, height // patch_size, width // patch_size
        self.base_size = height // patch_size
        self.interpolation_scale = interpolation_scale
    ...
    def cropped_pos_embed(self, depth,height, width):
        """Crops positional embeddings for SD3 compatibility."""
        if self.pos_embed_max_size is None:
            raise ValueError("`pos_embed_max_size` must be set for cropping.")
        depth = depth // self.patch_size
        height = height // self.patch_size
        width = width // self.patch_size
        if depth > self.pos_embed_max_size:
            raise ValueError(
                f"Depth ({depth}) cannot be greater than `pos_embed_max_size`: {self.pos_embed_max_size}."
            )
        if height > self.pos_embed_max_size:
            raise ValueError(
                f"Height ({height}) cannot be greater than `pos_embed_max_size`: {self.pos_embed_max_size}."
        ...
        d = (self.pos_embed_max_size - depth) // 2
        h = (self.pos_embed_max_size - height) // 2
        w = (self.pos_embed_max_size - width) // 2
        spatial_pos_embed = self.pos_embed.reshape(1, self.pos_embed_max_size,self.pos_embed_max_size, self.pos_embed_max_size, -1)
        spatial_pos_embed = spatial_pos_embed[:,d:d+depth, h : h + height, w : w + width, :]
        spatial_pos_embed = spatial_pos_embed.reshape(1, -1, spatial_pos_embed.shape[-1])
        return spatial_pos_embed
    ...
    def forward(self, latent):
        if self.pos_embed_max_size is not None:
            ...
            depth,height, width = latent.shape[-3] // self.patch_size, latent.shape[-2] // self.patch_size,latent.shape[-1] // self.patch_size
        ...
        # Interpolate or crop positional embeddings as needed
        if self.pos_embed_max_size:
            pos_embed = self.cropped_pos_embed(depth,height, width)
        else:
            if self.depth!=depth or self.height != height or self.width != width:
                pos_embed = get_3d_sincos_pos_embed(
                    embed_dim=self.pos_embed.shape[-1],
