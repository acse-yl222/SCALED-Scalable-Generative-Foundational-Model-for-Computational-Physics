...
class DiTTransformer3DModel(ModelMixin, ConfigMixin):
    r"""
    A 2D Transformer model as introduced in DiT (https://arxiv.org/abs/2212.09748).
    ...
    def __init__(
        ...
        self.proj_out_1 = nn.Linear(self.inner_dim, 2 * self.inner_dim)
        self.proj_out_2 = nn.Linear(
            self.inner_dim, self.config.patch_size * self.config.patch_size * self.config.patch_size * self.out_channels
        )
    ...
    def forward(
        ...
        """
        # 1. Input
        depth, height, width = hidden_states.shape[-3] // self.patch_size, hidden_states.shape[-2] // self.patch_size, \
        ...
        # unpatchify
        # depth = height = width = self
        hidden_states = hidden_states.reshape(
            shape=(-1, depth, height, width, self.patch_size, self.patch_size, self.patch_size, self.out_channels)
        )
        hidden_states = torch.einsum("ndhwkpqc->ncdkhpwq", hidden_states)
        output = hidden_states.reshape(
            shape=(-1, self.out_channels, depth * self.patch_size, height * self.patch_size, width * self.patch_size)
        )
