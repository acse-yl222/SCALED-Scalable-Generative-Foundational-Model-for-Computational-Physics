...
class SFCDiffusionDataset(Dataset):
    def __init__(
            self,
            data_dir='',
            data_list=range(1000),
...
class SFCDiffusionDatasetv1(Dataset):
    def __init__(
            self,
            data_dir='',
            data_list=range(1000),
...
#     def get_data(self,timestep):
#         result = np.zeros((20560,3))
#         data = np.loadtxt(os.path.join(self.data_dir, self.data_list[timestep]),delimiter=',')[:,2:]
#         data = data*10
#         data[:,0] = (data[:,0]-0.25)/2*5
#         data[:,1] = data[:,1]/2*5
#         result[:data.shape[0],:2] = data
#         position = [i for i in range(20560)]/20560
